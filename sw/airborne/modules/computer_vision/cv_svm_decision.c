...
static struct image_t *cam_callback(struct image_t *img __attribute__((unused))) {
  ...
  PRINT("Downsampled width %d pix\n", x_width);
  PRINT("Downsampled height %d pix\n", y_height);
  PRINT("Downsampled area: %d | N_features: %d\n", x_width * y_height, N_FEATURES);
  ...
  svm_decision_msg.decision = svm_decision_msg.value > 0 ? true : false;
...
{ 
  ...
  pthread_mutex_lock(&mutex);
  memcpy(&local_msg, &ground_filter_msg, sizeof(struct svm_decision_msg_t));
  pthread_mutex_unlock(&mutex);
  //PRINT("periodic");
